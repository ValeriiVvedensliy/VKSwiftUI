...
class MainCoordinator: Coordinator {
  ...
  func start() {
    ...
    let loginView = LoginView(isAuthorized: loginViewBinding)
  ...
  private func createFriendsController() -> UIViewController {
    let viewModel = UsersViewModel(service: NetworkFriendsVk())
    let friendsView = UsersView(viewModel: viewModel)
    return UIHostingController(rootView: friendsView)
  }
  private func createGroupsController() -> UIViewController {
    let viewModel = GroupViewModel(service: NetworkGrpupsVk())
    let groupsView = GroupView(viewModel: viewModel)
    return UIHostingController(rootView: groupsView)
  }
  private func createNewsfeedController() -> UIViewController {
    let newsViewModel = NewsViewModel(service: NetworkNewsVk())
    let newsfeedView = NewsView(viewModel: newsViewModel)
